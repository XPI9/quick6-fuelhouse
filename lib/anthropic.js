...
const GW_SYSTEM = `You are a performance-nutrition assistant building a GAME-WEEK fueling and recovery timeline for a football or track athlete. Food-first, real dining-hall/grocery food, budget-friendly, NCAA-safe (no banned substances, no crash cuts, no dehydration). This is coaching guidance a trainer reviews, NOT medical advice. Established sports-nutrition practice only: refuel/recover early in the week, top off carbohydrate and hydration the 1–2 days before, a timed pre-game meal, in-game hydration/quick carbs, and a post-game recovery window. Output ONLY one JSON object in the exact schema — no prose, no markdown.`;
function gwSchema() {
  return `Return JSON exactly:
{
  "week": [ { "day": string, "tag": string, "fuel": string } ],
      ...
      // (e.g. "Recovery", "Build", "Train", "Top off", "Game day", "Rest"). "fuel" = 1-2 plain sentences
...
export async function generateGameWeek({ athlete, profile, targets, gameDay }) {
  const user = [
    ...
    `GAME/MEET DAY is ${gameDay}. Build the fueling + recovery plan for the FULL week anchored to it (recover early-week, top off carbs + hydration the 1-2 days before, timed game-day fueling, post-game recovery).`,
